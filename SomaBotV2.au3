#RequireAdmin
Opt("TrayIconDebug", 1)
;~ #AutoIt3Wrapper_res_Compatibility=Windows7,Vista
...
;~ Global $priority = _ProcessGetPriority(@AutoItPID)
;~ MsgBox(1, "", @AutoItPID & @CRLF & $priority)
...
If @Compiled = 1 And @ScriptName = "SomaBot.exe" Then
	MsgBox(0, "", "SomaBot.exe has been renamed to " & $word & ".exe" & @CRLF & @CRLF & "Your EXE file has been renamed. Please restart the bot after it launches again in a moment")
...
$hWnd = _GetHwndFromPID($processId)

Global $NameOfSomaWindow = "Myth of Soma"
...
Global $speed[1] = [0x48]
Global $equippedweapoffset[1] = [0x2b0]
...
Global $MobSelectionSwitch = IniRead("Settings.ini", "Settings", "MobSelection", 3)
...
Global $HPpotionuse = IniRead("Settings.ini", "Settings", "HPPotionUse", 100) ; If your HP drops below this number then the game will use a HP pot from F1 - Careful using this with a new char, make sure your HP is aboove this number!
Global $MPpotionuse = IniRead("Settings.ini", "Settings", "MPPotionUse", 50) ; If your MP drops below this number then the game will use a MP pot from F2
Global $HPToWarp = IniRead("Settings.ini", "Settings", "HPToWarp", 40) ; If your MP drops below this number then the game will use a MP pot from F2
Global $HealSpellSlot = IniRead("Settings.ini", "Settings", "HealSpellSlot", 0) ; 0,1,2,3 for F5 f6 f7 f8 -
Global $StaffAuraSlot = IniRead("Settings.ini", "Settings", "StaffAuraSlot", 3) ; 0,1,2,3 for F5 f6 f7 f8 -
Global $AttackSpellSlot = IniRead("Settings.ini", "Settings", "AttackSpellSlot", 1) ; 0,1,2,3 for F5 f6 f7 f8 -
Global $WeakenSpellSlot = IniRead("Settings.ini", "Settings", "WeakenSpellSlot", 2) ; 0,1,2,3 for F5 f6 f7 f8 -
...
Global $MeleeAuraSwitch = IniRead("Settings.ini", "Settings", "MeleeAuraSwitch", 1)
Global $autoheal = IniRead("Settings.ini", "Settings", "AutoHeal", 0)
Global $F3Trash = IniRead("Settings.ini", "Settings", "F3Trash", 1)
$Map2 = 0
...
Func FillEntityArray()
	...
	If $LicenseArray[$ChecksumPlacement][$ChecksumPlacement] = $Checksum Then
		...
		$equippedweap = _KDMemory_ReadProcessMemory($handles, 0x06d8600, 'DWORD', $equippedweapoffset) ; What weap? 6 = staff
		Global $RecentDeadMob = _KDMemory_ReadProcessMemory($handles, 0x06d8dd4, 'DWORD', $UniqueID)
	EndIf
...
;~ 		$EA1[$i][11] = _KDMemory_ReadProcessMemory($handles, $EntityAddressHex, 'DWORD', $GreyHPBars)
		...
		$EA1[$i][12] = _KDMemory_ReadProcessMemory($handles, $EntityAddressHex, 'DWORD', $equippedweapoffset)
...
;~ 		$EA2[$i][11] = _KDMemory_ReadProcessMemory($handles, $EntityAddressHex2, 'DWORD', $GreyHPBars)
		...
		$EA2[$i][12] = _KDMemory_ReadProcessMemory($handles, $EntityAddressHex2, 'DWORD', $equippedweapoffset)
...
Func StaffAura()
	If $StaffAuraToggle = 1 Then
		...
		If $staffaura = 0 And $stance <> 0 And $equippedweap = 6 And WinActive($NameOfSomaWindow) Then
...
;~ 		If $EA[$i][0] = $MobDropArray[10][0] And $EA[$i][7] = $MobDropArray[10][1] And $NavigatingToDrop = False And $EA[$i][0] <> 0 And $EA[$i][5] = 15 Then Add and mobhp =
...
Func F3Trash()
	If $F3Number > 1 And $F3Trash = 1 Then
		If $F3Number <> $F3Last Then
			$F3Last = $F3Number
			If Not WinActive($NameOfSomaWindow) Then
				WinActivate($NameOfSomaWindow)
				WinWaitActive($NameOfSomaWindow)
...
Func AutoHeal()
	If $autoheal = 1 And $boton = 1 Then
		If $HPCurrent < $HPMax - 30 And $NavigatingToDrop = False Then
			For $i = 1 To 38 Step 1
				If $EA[$i][26] = $MyID And $EA[$i][5] <> 15 Then
					ExitLoop
				ElseIf $i = 38 And $EA[$i][26] <> $MyID Then
					$StopHeal = False
					$slot = _KDMemory_WriteProcessMemory($handles, 0x05366ac, 'DWORD', $HealSpellSlot, $offsets = 0)
					Do
						If Not WinActive($NameOfSomaWindow) Then
							WinActivate($NameOfSomaWindow)
							WinWaitActive($NameOfSomaWindow)
						EndIf
						For $i = 1 To 38 Step 1
							If $EA[$i][26] = $MyID And $EA[$i][5] <> 15 And $EA2[$i][24] <> 7 Then
								$StopHeal = True
							EndIf
						Next
						$EntityCursorTooltip = _KDMemory_ReadProcessMemory($handles, 0x06d85F8, 'DWORD', $offsets = 0) ;read the ID of the mob possibly under the cursor
						If $EntityCursorTooltip = 4294967295 Then ; if it is not a mob or player
							MouseClick("right")
						Else
							MouseMove(Random($Reswidth - 250, $Reswidth + 250, 0), Random($Resheight - 250, $Resheight + 250, 0), 0)
						EndIf
						Sleep("20")
						$HPCurrent = _KDMemory_ReadProcessMemory($handles, 0x06d8600, 'DWORD', $HPoffsets)
						$MPCurrent = _KDMemory_ReadProcessMemory($handles, 0x06d8600, 'DWORD', $MPoffsets)
;~ 						If $autoheal = 1 Then
;~ 							HotKeySet("!o", "autoheal")
;~ 						EndIf
						FillEntityArray()
						Potions()
;~ 					Until $HPCurrent >= ($HPMax - 10) Or $MPMax < 3 Or $autoheal = 0 Or $Manualheal = 0
						If $HPCurrent >= ($HPMax - 20) Then
							Sleep(500)
						EndIf
					Until $HPCurrent >= ($HPMax - 20) Or $MPMax < 3 Or $autoheal = 0 Or $StopHeal = True
;~ 					$slot = _KDMemory_WriteProcessMemory($handles, 0x05366ac, 'DWORD', $currentslot, $offsets = 0)
;~ 					Sleep(1000)
				EndIf
			Next
...
Func Gui()
	...
	Global $AutoHPPotsLabel = GUICtrlCreateLabel("Auto HP Pots(BROKE!): " & $AutoHPPots, 184, 72, 156, 17)
	Global $AutoMPPotsLabel = GUICtrlCreateLabel("Auto MP Pots(BROKE!): " & $AutoMPPots, 184, 89, 157, 17)
	...
	Global $AutoHealLabel = GUICtrlCreateLabel("AutoHeal(ALT + ]): " & $autoheal, 184, 225, 157, 17)
	GUICtrlSetResizing(-1, $GUI_DOCKTOP)
	GUICtrlCreateGroup("", -99, -99, 1, 1)
	Global $SaveProfileButton = GUICtrlCreateButton("Save BROKE!", 176, 8, 75, 25)
...
Func AutoHealSwitch()
	If $autoheal = 0 Then
		$autoheal = 1
		GUICtrlSetData($AutoHealLabel, "AutoHeal(ALT + ]): " & $autoheal)
	ElseIf $autoheal = 1 Then
		$autoheal = 0
		GUICtrlSetData($AutoHealLabel, "AutoHeal(ALT + ]): " & $autoheal)
	EndIf
EndFunc   ;==>AutoHealSwitch
